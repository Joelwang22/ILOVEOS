...
handle = win32file.CreateFile(path, access, 0, None,
  ...
  pywin32Modules: [
    ...
    },
    {
      ...
      features: [
        { name: "WAIT_OBJECT_0", task: "Recognise a satisfied single-object wait", detail: "For one handle, this means the object's documented signaled condition satisfied the wait. For a mutex, it also means the caller acquired ownership normally." },
        { name: "WAIT_TIMEOUT", task: "Handle an expired wait deadline", detail: "The wait function completed successfully but no selected object satisfied it before the timeout. Nothing was acquired, so do not release a mutex or semaphore permit." },
        { name: "WAIT_ABANDONED", task: "Recover from an abandoned mutex", detail: "The caller acquired the mutex after its previous owning thread terminated. Treat protected state as potentially inconsistent and release only after validation or safe rejection." },
        { name: "WAIT_FAILED", task: "Recognise a native wait failure", detail: "The native API failed rather than reporting an object state. pywin32 normally raises pywintypes.error for this condition, while direct ctypes code must retrieve the last error immediately." },
        { name: "INFINITE", task: "Request an unbounded wait", detail: "Wait without a timeout only when another shutdown or wake path is guaranteed. Interactive and service code usually benefits from cancellation or bounded diagnostics." },
    ...
    },
    {
      ...
      constants: ["ERROR_BROKEN_PIPE", "ERROR_PIPE_CONNECTED", "ERROR_MORE_DATA", "ERROR_PIPE_BUSY", "ERROR_NO_DATA", "ERROR_FILE_NOT_FOUND", "ERROR_ACCESS_DENIED", "ERROR_INSUFFICIENT_BUFFER"],
      features: [
        ...
        { name: "ERROR_MORE_DATA", task: "Continue a partial message-mode pipe read", detail: "The read buffer held only part of one message. Retain the returned bytes, enforce a maximum total size, and continue until that message completes." },
        { name: "ERROR_PIPE_BUSY", task: "Handle unavailable named-pipe instances", detail: "All server instances are busy. WaitNamedPipe and a bounded retry policy can wait for availability, but the later CreateFile can still race." },
        { name: "ERROR_NO_DATA", task: "Recognise a closing or disconnected pipe", detail: "The peer or endpoint state does not permit the requested transfer. Distinguish it from a complete protocol response and clean up the current exchange." },
    ...
    },
    {
      ...
      features: [
        ...
        { name: "SetWindowsHookExW", task: "Install a documented Windows event hook", detail: "Choose an exact hook type and thread or desktop scope, provide an ABI-correct callback and module arrangement, retain callback lifetime, and match the returned HHOOK with UnhookWindowsHookEx." },
        { name: "CallNextHookEx", task: "Continue a Windows hook chain", detail: "Pass the current hook event parameters onward when the selected hook contract requires it. Do not block, corrupt parameters, or silently break unrelated hooks." },
        { name: "UnhookWindowsHookEx", task: "Remove an installed Windows hook", detail: "Prevents future delivery through the HHOOK. Coordinate in-flight callbacks before releasing callback storage or unloading its DLL." },
        { name: "VirtualAllocEx", task: "Allocate pages in another process", detail: "Requires a process handle with PROCESS_VM_OPERATION. The returned address belongs to the target process, and allocation state must be matched by a documented VirtualFreeEx cleanup path." },
        { name: "WriteProcessMemory", task: "Copy bytes into another process", detail: "Requires target access and a valid writable range. Check BOOL and bytes-written output separately, and restrict any use to an owned purpose-built lab process." },
        { name: "VirtualFreeEx", task: "Release or decommit remote pages", detail: "Use the target process handle, original allocation base, and a size/free-type combination matching the remote allocation contract." },
        { name: "CreateRemoteThread", task: "Create a thread in another process", detail: "Requires a valid target-process start address, compatible architecture and ABI, appropriate process rights, thread-result handling, and strict authorization. This course treats it as a defensive evidence contract." },
        { name: "FlushInstructionCache", task: "Synchronize modified or generated instructions", detail: "After code bytes are produced or changed under a supported design, flush the affected process range before execution as required by the documented coherency contract." },
        { name: "RtlAddFunctionTable", task: "Register dynamic x64 unwind metadata", detail: "Associates runtime-function entries with a dynamic code range. The table and referenced code must remain valid until removed with the matching runtime-function-table operation." },
        { name: "InterlockedExchangePointer", task: "Atomically replace a pointer-sized value", detail: "The destination must be correctly aligned and writable. Atomic pointer replacement does not by itself solve callback lifetime, active readers, recursion, or rollback." },
        { name: "WinVerifyTrust", task: "Ask a trust provider to verify a file", detail: "Build the documented action GUID and WINTRUST_DATA input, interpret LONG status under that contract, and close any provider state. A valid signature is provenance evidence, not a behavioral verdict." },
