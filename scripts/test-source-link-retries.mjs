...
const response = (status) => ({ status, ok: status >= 200 && status < 300, body: null });
let calls = 0;
const transient = [response(503), response(429), response(204)];
const recovered = await checkSourceLink(item, { request: async () => { calls += 1; return transient.shift(); }, sleep: async () => {} });
