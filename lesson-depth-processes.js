window.ILOVEOS_LESSON_DEPTH = {
  ...
  "programs-processes-isolation": {
    ...
    practice: {
      ...
      steps: [
        { action: "Start two Notepad processes and record which one was started first.", why: "A controlled creation order gives the two otherwise similar instances a known distinguishing fact.", observe: "Confirm two live rows with different PIDs and creation times." },
        { action: "Open each process's Image and Performance properties in Process Explorer.", why: "These views separate file identity from runtime accounting and private state.", observe: "Compare path, command line, parent, start time, private bytes, working set, handle count, and thread count." },
        { action: "Compare the DLL and handle lower-pane views.", why: "Modules are mapped code images, while handles are checked references to objects. Similar-looking rows answer different questions.", observe: "Find several shared module paths and at least one handle or object unique to each process." },
      ...
      hints: [{ title: "Notepad reused an existing process", body: "Recent Notepad versions can behave differently across builds. If two starts do not produce two rows, use two instances of a simple supplied Python pause script instead." }],
  ...
  "process-address-space": {
    ...
    practice: {
      ...
      steps: [
        { action: "Download the starter, run py process_inventory_lab.py, then leave it paused at the first prompt.", why: "The supplied process creates known state, giving every external observation a ground-truth explanation.", observe: "Record PID, temporary path, Python architecture, worker native TID, and allocated byte count." },
        { action: "Open the process properties and both lower-pane modes in Process Explorer.", why: "Handles and DLLs must be inspected as separate resource categories.", observe: "Find the temporary file handle, several Python or Windows modules, the worker TID, start time, and current memory counters." },
        { action: "Open the process in VMMap and classify representative regions.", why: "Virtual memory needs a region map rather than one total counter.", observe: "Locate Image, Heap, Stack, and Private Data categories and record one base address from each." },
        { action: "Continue the script so it closes the file and releases the worker, then refresh both tools.", why: "A controlled state transition tests which resource belongs to which lifetime.", observe: "The file handle disappears and the worker thread ends, while the process image, main thread, and remaining memory stay until final exit." },
        { action: "Finish the script and write a scope table.", why: "The table converts tool rows into a reusable process model.", observe: "Classify each observation as file, mapping, process-wide state, thread state, security context, or measurement." }
      ],
  ...
  "context-switch-process-metadata": {
    ...
    practice: {
      ...
      steps: [
        { action: "Create two controlled input directories, one tiny and one containing many harmless copied text files.", why: "Controlled file counts avoid scanning sensitive or changing system directories and make repeated runs comparable.", observe: "Record file count and total bytes for each directory." },
        { action: "Run py thread_io_lab.py tiny 1 and py thread_io_lab.py tiny 5, repeating each command three times.", why: "The final argument selects worker count, and the small case exposes fixed thread and executor overhead.", observe: "Record the three elapsed times per worker count and compare medians." },
        { action: "Run py thread_io_lab.py many 1, then repeat with 5, adding --pause while capturing the test PID in Process Monitor.", why: "The larger workload tests whether overlapping file waits can repay the fixed overhead.", observe: "Show TID, CreateFile, ReadFile, and CloseFile, then record how many worker TIDs issued reads." },
      ...
      cleanup: ["Delete only the controlled input directories you created for the lab.", "Stop and clear the Process Monitor capture, then close the test script."],
  ...
  "createprocess-lifecycle": {
    ...
    practice: {
      ...
      steps: [
        ...
        { action: "Run py create_process_lab.py and record the returned Notepad identifiers.", why: "The visible case establishes ordinary creation and correlation before presentation flags are changed.", observe: "Find the child PID, initial TID, parent PID, image path, and creation time in Process Explorer." },
        { action: "Close the child and follow the launcher's wait and exit-code output.", why: "Waiting on the handle demonstrates process-object signaling and avoids process-name polling.", observe: "Confirm WAIT_OBJECT_0, a final exit code, and cleanup messages for both handles." },
        { action: "Run py create_process_lab.py --hide and inspect the process externally.", why: "The comparison separates window presentation from process existence.", observe: "The initial window behavior may differ by target, but the process, thread, token, and resource state remain observable." },
        { action: "Run py create_process_lab.py C:\\ILOVEOS\\does-not-exist.exe to trigger one safe failure.", why: "The failure path proves last-error handling without creating partial ownership.", observe: "Record the numeric Windows error and confirm that no output handle is closed as though creation had succeeded." }
      ],
  ...
  "object-manager": {
    ...
    practice: {
      ...
      steps: [
        { action: "In the first terminal, run py named_event_lab.py creator and leave it paused.", why: "The pause makes the object and creator handle long-lived enough for snapshot tools.", observe: "Record the creator PID, printed handle value, object name, and initial nonsignaled state." },
        { action: "Locate the event in WinObj and the creator's handle in Process Explorer.", why: "The two observations distinguish namespace presence from per-process ownership.", observe: "Confirm Event type and correlate the exact creator PID." },
        { action: "In a second terminal, run py named_event_lab.py opener.", why: "OpenEvent demonstrates name lookup, access checking, and a separate handle table entry.", observe: "Record the second PID and handle value; do not expect the numeric handles to match." },
      ...
      cleanup: ["Exit both starter processes through their prompts so each closes its handle.", "Verify no test process remains; the named event needs no file cleanup."],
  ...
  "handles-rights-lifetime": {
    ...
    practice: {
      ...
      steps: [
        { action: "Run py file_lifetime_lab.py keep-open --records 25, then repeat with reopen and flush as the mode.", why: "Correctness must be established before timing or event-count comparisons are meaningful.", observe: "Confirm the same number and order of records in iloveos_keep-open.log, iloveos_reopen.log, and iloveos_flush.log." },
        { action: "Capture py file_lifetime_lab.py keep-open --records 250 with PID and exact output-path filters.", why: "This establishes the minimal logical handle-lifetime sequence.", observe: "Count CreateFile, WriteFile, FlushBuffersFile, and CloseFile events, including runtime or metadata operations you did not predict." },
        { action: "Repeat the 250-record capture with reopen as the mode.", why: "Changing only handle lifetime isolates repeated open and close behavior.", observe: "Show the repeated CreateFile and CloseFile pattern and compare total elapsed time across several runs." },
        { action: "Repeat the 250-record capture with flush as the mode, keeping handle churn separate from flush count.", why: "A durability barrier is a different design choice from repeated acquisition.", observe: "Look for FlushBuffersFile after writes while the main handle remains open." },
      ...
      cleanup: ["Delete only the three temporary log files created by the starter.", "Stop and clear the Process Monitor capture after saving any notes you need."],
