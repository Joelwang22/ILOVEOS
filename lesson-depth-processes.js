window.ILOVEOS_LESSON_DEPTH = {
  ...
  "programs-processes-isolation": {
    ...
    learning: [
      ...
      },
      {
        ...
        inlineCheck: ["Which information most reliably identifies one process instance during a short investigation?", ["Executable name alone", "PID alone after the process exits", "PID paired with creation time or a live process handle", "The parent process name"], 2, "PIDs are reused. Creation time or a live handle distinguishes the instance that owned the PID during the observation."]
      }
    ],
    visuals: [
      {
        ...
        caption: "Another process built from the same image has its own PID, address space, handle table, and lifetime."
      }
    ],
    workedExamples: [
      {
        ...
        conclusion: "A program describes possible execution. A process is one protected, temporary instance of that execution."
      }
    ],
    windowsLearning: [
      {
        title: "Enumeration finds candidates; OpenProcess requests access",
        paragraphs: [
    ...
    practice: {
      ...
      steps: [
        {
          action: "In PowerShell, launch Notepad twice and print the process ID returned by each launch.",
          commands: [
          ...
          why: "A controlled creation order gives the two otherwise similar processes one known distinguishing fact.",
          observe: "Two live Notepad rows normally appear with different PIDs and creation times. If this Windows build opens both windows in one host process, use the two labeled fallback commands in separate PowerShell windows and keep both waiting."
        },
        ...
        { action: "Select each PID printed in step 1, choose View > Show Lower Pane, and switch View > Lower Pane View between DLLs and Handles.", why: "Modules are mapped code images, while handles are access-checked references to objects.", observe: "The DLLs view shows shared module paths, while the Handles view shows process-local object references. Access denial and an exited PID are distinct visible outcomes." },
      ...
      hints: [{ title: "Notepad reused an existing process", body: "Recent Notepad versions can behave differently across builds. If two launches do not produce two rows, run the two labeled py -c fallback blocks in separate PowerShell windows." }],
  ...
  "process-address-space": {
    ...
    workedExamples: [
      {
        ...
        shared: "A thread can create or close a process-wide resource, so the program must define an ownership policy even when Windows stores the resource at process scope.",
    ...
    windowsLearning: [
      ...
      {
        title: "A process handle provides checked access to a PID",
        paragraphs: [
          ...
          "Close the handle in a finally block. The PyHANDLE object can usually close itself when collected, but deterministic cleanup makes its lifetime visible and prevents large inventories from temporarily accumulating hundreds of open process handles. Skip access-denied processes and record the reason instead of allowing one denial to fail the entire enumeration."
        ]
      }
    ],
    codeWalkthroughs: [
      {
        ...
        intro: "The API sequence separates identity, access checks, queries, and cleanup.",
        stages: [
          ...
          { title: "Release the caller's reference", explanation: "The finally block runs whether the query succeeds, the process exits, or later formatting raises an exception.", code: "finally:\n    handle.Close()" }
        ]
      }
    ],
    practice: {
      ...
      steps: [
        {
          ...
          observe: "process_inventory_lab.py prints its PID, held-open path, Python architecture, main native TID, worker native TID, and a 16,777,216-byte private buffer. If the py launcher is unavailable, resolve the Python setup problem before continuing."
        },
        { action: "Select the PID printed by process_inventory_lab.py in Process Explorer. Inspect Properties > Image, Performance, and Threads; then choose View > Show Lower Pane and switch View > Lower Pane View between Handles and DLLs.", why: "Handles and DLLs are separate resource categories.", observe: "The Handles view shows the exact held-open path, the DLLs view shows Python and Windows modules, and Properties > Threads shows the printed worker TID. Access denial and an exited PID are distinct outcomes." },
        ...
        { action: "Press Enter once in process_inventory_lab.py so it closes the held-open file and signals its worker to exit. Leave the program at 'Refresh the tools,' then refresh Process Explorer and VMMap for the same PID.", why: "A controlled transition exposes the file and worker lifetimes while the process remains live.", observe: "The exact held-open path and printed worker TID disappear, while the process image, main TID, and remaining memory stay visible until the final exit." },
        { action: "Press Enter a second time in process_inventory_lab.py so it exits and removes its temporary directory; refresh Process Explorer once more.", why: "The supplied program owns the temporary path and performs its own cleanup.", observe: "The command returns to PowerShell, the printed PID is no longer live, and the temporary directory has been removed." }
      ],
      ...
      cleanup: ["If process_inventory_lab.py is still paused, press Enter until it exits and removes its temporary directory.", "Close the Process Explorer and VMMap views opened for the lab."]
    },
    checks: [
      ...
      ["What should an enumeration tool do when one protected process returns access denied?", ["Disable Windows protections", "Stop and discard every earlier result", "Record the limitation and continue where possible", "Request every right from every process"], 2, "An access failure is an expected outcome in a mixed process inventory. Least privilege and partial results are preferable to weakening protections."]
    ]
  },
  ...
  "context-switch-process-metadata": {
    ...
    learning: [
      {
        ...
        paragraphs: [
          ...
          "A switch between threads in the same process can retain the same address-space context, while a cross-process switch changes which virtual mappings are active. Modern processors cache translations and state, but switches still incur direct save-and-restore costs as well as indirect cache and locality costs. A large switch count does not tell you whether the switches enabled useful overlap or reflected excessive contention."
        ]
      },
      {
        ...
        paragraphs: [
          ...
          "A live process or thread handle is stronger than a bare number because it refers to the opened object. Even then, the object may have terminated, so code must handle that state. For saved evidence, record capture time and creation time rather than assuming a number has permanent meaning."
        ]
      },
      {
    ...
    visuals: [
      {
        ...
        items: [
          ...
          { meta: "Preserve", label: "Save A's context", detail: "A begins waiting", linkAfter: "dispatch" },
          ...
          { meta: "Resume", label: "Restore A's context", detail: "A continues after its wait" }
        ],
    ...
    workedExamples: [
      {
        ...
        steps: [
          { title: "Calculate the observed speedup", action: "Divide the single-worker time by the multi-worker time.", why: "A ratio communicates the measured elapsed-time change for this run.", result: "4.0 / 1.2 = a speedup of about 3.33×." },
          { title: "State what the trace establishes", action: "Identify that several TIDs issued overlapping sequences of file operations.", why: "TID evidence connects operations to workers and shows concurrency in the request stream.", result: "The trace shows that multiple threads issued I/O requests concurrently." },
          ...
          { title: "Limit the conclusion", action: "State that the result applies to this directory, cache state, machine, and worker count.", why: "The result from the supplied better_together exercise is experimental, not a universal constant.", result: "Five workers helped this tested workload under these conditions." }
        ],
    ...
    windowsLearning: [
      ...
      },
      {
        ...
        paragraphs: [
          "Run the same workload several times for each worker count and record the median. Keep the file set, output behavior, antivirus conditions, power mode, and measurement boundaries consistent. The first run may populate the file cache, so decide whether the question concerns cold or warm behavior and report it.",
    ...
    practice: {
      ...
      expectedOutcome: "A small file set may be slower with several workers because startup and coordination dominate. A larger cached or storage-backed set may improve when reads overlap, but the best count depends on the machine and files. Process Monitor should attribute reads to more worker TIDs in multi mode. Interleaving shows concurrent requests, not guaranteed physical-disk or CPU parallelism.",
      steps: [
        {
          action: "Download thread_io_lab.py and open PowerShell in its folder. Create the .\\iloveos-io-tiny and .\\iloveos-io-many input directories for the lab with this complete block.",
        ...
        },
        {
          ...
          observe: "Each run prints the root path, a file count of 2, the selected worker count, the number of bytes read, and a machine-dependent elapsed time. If a dependency or path error occurs, resolve it before continuing."
        },
        {
          action: "For the many-file input, run the complete one-worker command, then repeat the full setup with the five-worker command. At each thread_io_lab.py pause, add two Include filters in Process Monitor: PID is the printed PID, and Operation is ReadFile. Clear the display, resume capture, press Enter once, and pause capture when the script prints the elapsed time.",
          ...
          observe: "Process Monitor attributes ReadFile rows to the printed PID. The five-worker run can show several TIDs issuing reads, while exact ordering and elapsed values vary. If no ReadFile rows appear, disable only the Operation filter to determine whether that filter hid the captured activity."
        },
        {
          action: "After both thread_io_lab.py processes exit, run this PowerShell cleanup block from the folder containing the two lab-created input directories.",
          ...
          observe: "PowerShell prints False for both lab-created paths. Any True result means that specific directory remains."
        }
      ],
      hints: [{ title: "The multi-worker run is slower", body: "That is a valid machine-dependent result. The goal is to attribute ReadFile activity, not to guarantee a speedup." }],
      cleanup: ["Leave Process Monitor capture stopped and clear only the events displayed for this lab.", "The final PowerShell block removes only .\\iloveos-io-tiny and .\\iloveos-io-many."]
    },
    checks: [
  ...
  "createprocess-lifecycle": {
    ...
    learning: [
      ...
      {
        title: "Inheritance must be enabled on both the handle and the creation call",
        paragraphs: [
    ...
    workedExamples: [
      {
        ...
        steps: [
          ...
          { title: "Initialize inputs", action: "Set startup.cb, use an explicit executable path, and create a writable command-line buffer.", why: "CreateProcessW validates cb and may modify command-line storage.", result: "Every required input remains allocated and writable throughout the call." },
          { title: "Branch on creation", action: "Test the returned BOOL and capture ctypes.get_last_error immediately if it is zero.", why: "A failed call does not initialize valid output handles.", result: "The caller takes ownership of the handles only on the success path." },
          { title: "Observe and wait", action: "Use the PID for tool correlation and the process handle for WaitForSingleObject and GetExitCodeProcess.", why: "The handle provides access-checked, waitable identity, while the PID is only a number.", result: "The child's lifecycle becomes visible without polling process names." },
          { title: "Release both references", action: "Close hThread and hProcess in a finally block, including on error paths after successful creation.", why: "The creator owns both returned handles even after the child exits.", result: "The process and thread objects can be deleted when all other references are gone." }
        ],
        conclusion: "Correct process creation includes managing the returned handles, not merely checking for a true return value."
      }
    ],
    windowsLearning: [
      {
    ...
    practice: {
      ...
      expectedOutcome: "The launcher should create Notepad or another explicit harmless target, print the child PID and TID, optionally request a hidden initial window, and keep the process handle until the child exits. Process Explorer should reveal the child in either window mode. The wait should return WAIT_OBJECT_0, the exit code should become available, and both returned handles should close in a finally block.",
      steps: [
        { action: "Download create_process_lab.py and open the complete file in a text viewer before running it.", why: "The supplied program lets you inspect its ABI declarations before it calls native code.", observe: "create_process_lab.py contains full STARTUPINFOW and PROCESS_INFORMATION fields, pointer-sized HANDLE members, startup.cb, WinDLL, use_last_error, argtypes, and restype." },
        {
          ...
          why: "The default target uses the explicit System32\\notepad.exe path. This establishes the normal creation and correlation behavior before any window-presentation flags change.",
          observe: "create_process_lab.py prints the child PID, initial TID, and 'hide requested: False.' Process Explorer shows that exact child PID, its parent, the System32\\notepad.exe image path, and its creation time."
        },
        ...
        {
          action: "Run the complete hidden-window command and leave create_process_lab.py at its inspection prompt before locating the printed child PID in Process Explorer.",
        ...
        {
          action: "After closing the hidden test child and allowing the launcher to finish, run this guarded command to test a missing target.",
          ...
          why: "The failure path demonstrates last-error handling without creating handles that would require cleanup.",
          observe: "The command reports a numeric Windows error for the missing path and prints neither a child PID nor successful handle-cleanup messages. A collision or access-denied error is different from the intended file-not-found result and leaves unknown files untouched."
        }
      ],
    ...
    learning: [
      {
        ...
        paragraphs: [
          ...
          "User-mode code does not receive a raw kernel address. It receives a handle value interpreted in the caller's process handle table. The table entry refers to the object and stores the granted access plus attributes such as inheritance. The same numeric value in another process can refer to a different entry or be invalid."
        ]
      },
      {
        ...
        inlineCheck: ["What lets a second process operate on an existing named event?", ["Knowing the name alone always grants full control", "Opening the event with rights permitted by its security settings", "Using the creator's numeric handle value directly", "Sharing the same executable path"], 1, "The name supports lookup, but Windows still performs an access check and returns a new handle in the second process."]
      },
      {
    ...
    workedExamples: [
      {
        ...
        steps: [
          ...
          { title: "Open", action: "Process B calls OpenEvent with SYNCHRONIZE access.", result: "B receives its own handle to the same event object.", why: "Name lookup and the security check produce a new handle for process B." },
          ...
          { title: "Close B", action: "Process B releases the final handle.", result: "The object and temporary name disappear when no other reference remains.", why: "Executive objects are deleted after the last relevant reference is released." }
        ],
    ...
    windowsLearning: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "The documentation for the creating or opening API defines ownership. Plan cleanup when acquiring the resource; do not infer it from how the Python object is printed."
        ]
      }
    ],
    practice: {
      ...
      time: "25 min",
      intro: "Use a program with creator and opener modes to share one named event while WinObj and Process Explorer show its namespace and handle views.",
      ...
      steps: [
        {
          ...
          observe: "named_event_lab.py prints 'mode: creator,' the creator PID, the name Local\\ILOVEOS_ObjectLab, and a process-local handle value. If pywin32 is unavailable, resolve the dependency problem before continuing."
        },
        { action: "In Process Explorer, choose View > Select Columns > Process Image, enable Session, and read the Session value for the creator PID printed in step 1. In WinObj, open \\Sessions, open the numeric directory that matches that Session value, open BaseNamedObjects, and select ILOVEOS_ObjectLab.", why: "The live process supplies the current session number used in the Object Manager namespace path.", observe: "WinObj shows an Event named ILOVEOS_ObjectLab. Some WinObj versions present the current session directly under \\BaseNamedObjects; access denial and a missing object are distinct outcomes." },
        { action: "In Process Explorer, select the creator PID printed in step 1, choose View > Show Lower Pane and View > Lower Pane View > Handles, then find the Event row whose name ends with ILOVEOS_ObjectLab.", why: "The handle lower pane shows the creator's ownership separately from the WinObj namespace entry.", observe: "The Handles view shows an Event row ending in ILOVEOS_ObjectLab for the exact creator PID while the first pause remains active." },
        {
          ...
          observe: "Opener mode prints a second PID, the same fixed name, and its own process-local handle value. The numeric values need not match. Windows error 2 means the named object is missing, whereas access denied reports a different problem."
        },
  ...
  "handles-rights-lifetime": {
    ...
    learning: [
      ...
      {
        title: "Closing a handle releases one reference, not the resource everywhere",
        paragraphs: [
      ...
      },
      {
        ...
        paragraphs: [
          "Keeping a file handle open avoids repeated path parsing, security checks, object creation or opening, and handle-table churn. Reopening the file for every record repeats that overhead. This was the first GoodLog and BadLog observation. It demonstrates an inefficient sequence of logical operations, not a direct count of physical disk writes.",
          "FlushFileBuffers asks the storage stack to push buffered data toward durable media. Calling it after every record can defeat batching and greatly increase latency, which was the second Good and Bad observation. Flushing is sometimes required by a durability contract, so the lesson is not to avoid it entirely. Choose a durability boundary deliberately and measure its cost separately from handle lifetime."
        ]
      }
    ],
    visuals: [
      {
        ...
        items: [
          ...
          { meta: "Use", label: "Read, write, query, wait", detail: "Only operations allowed by granted access", linkAfter: "when finished" },
    ...
    workedExamples: [
      {
        ...
        columns: [
          ...
          { title: "Reopen for every record", rows: [["Sequence", "Repeated CreateFile, WriteFile, CloseFile"], ["Handle churn", "High"], ["Buffering", "Closing the handle does not guarantee durability on every storage stack"], ["Risk", "Extra lookup and access-check work without a clear durability contract"]] }
        ],
    ...
    windowsLearning: [
      {
        title: "Process Explorer and Handle show who owns the reference now",
        paragraphs: [
      ...
      },
      {
        ...
        paragraphs: [
          "The first supplied report correctly distinguishes one long-lived open from repeated open, write, and close sequences. The explanation should add that path resolution, security checks, object and handle operations, runtime buffering, and storage caching all contribute, so Process Monitor alone cannot assign an exact duration to each layer.",
          "The second report correctly identifies FlushBuffersFile after each write as a likely source of latency. The result depends on the machine and storage device, and a flush request does not prove exactly when the physical media became durable. Report the elapsed time, operation sequence, storage conditions, and the application's stated durability requirement together."
        ]
      }
    ],
    practice: {
      ...
      steps: [
        {
          ...
          observe: "At each 'Start the filtered Process Monitor capture' prompt, press Enter once. Every mode prints 'records: 25,' and all three files contain the same fixed sequence of entries. A collision error protects pre-existing files."
        },
        {
          action: "Run the complete keep-open capture command. At its pause, add two Include filters in Process Monitor: PID is the printed PID, and Path is the printed full path. Clear the display, resume capture, press Enter to start timing, and pause capture when the program prints the elapsed time.",
          ...
          observe: "For the printed PID and exact path, Process Monitor shows one main open interval surrounding the writes. The program prints 'mode: keep-open,' 'records: 250,' the byte count, and a machine-dependent elapsed time."
        },
        {
        ...
        },
        {
          ...
          observe: "For the newly printed PID and path, Process Monitor shows FlushBuffersFile activity without the repeated open-and-close cycle seen in reopen mode. The exact elapsed time remains machine-dependent."
        },
        {
          ...
          why: "Cleanup targets only the three explicit files created by the lab.",
          observe: "PowerShell prints False three times. A True result identifies a lab-created log that still remains."
        }
      ],
      ...
      cleanup: ["Leave Process Monitor capture stopped and clear only the events displayed for this lab.", "The final PowerShell block removes only the three explicit ILOVEOS log files."]
    },
    checks: [
