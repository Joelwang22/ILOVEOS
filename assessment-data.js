(() => {
  ...
  const finalCases = [
    {
      id: "case-process-identity",
      title: "The process that changed underneath you",
      modules: ["foundations", "processes-handles"],
      summary: "Repair a native wrapper, preserve its real failure, and decide whether a reused PID still identifies the lab process.",
      scenario: "A diagnostic stores the PID of a short-lived lab process, then reopens that number several minutes later. The open is denied, and the error shown by the wrapper does not match the Process Monitor event.",
      artifacts: [
        { label: "Wrapper", kind: "code", content: "handle = OpenProcess(PROCESS_ALL_ACCESS, False, saved_pid)\nif not handle:\n    CloseHandle(snapshot)\n    raise ctypes.WinError()" },
        { label: "Timeline", kind: "log", content: "12:14:08  lab_target.exe  PID 4180  created\n12:14:21  lab_target.exe  PID 4180  exited\n12:19:47  updater.exe     PID 4180  created\n12:20:03  OpenProcess(4180, ALL_ACCESS) -> ACCESS DENIED" }
      ],
      questions: [
        single("case1-native-failure", "foundations", "failure", "last-error preservation", "The wrapper reports the error only after calling CloseHandle on another object. Which repair preserves the failure produced by OpenProcess?", ["Test the documented failure sentinel, capture the error immediately, then perform cleanup and report the saved error", "Call GetLastError before OpenProcess and reuse that value if the open fails", "Ignore the OpenProcess result and decide success from whether snapshot cleanup succeeds", "Request a pointer-sized result type and treat every non-null Python object as a valid process handle"], 0, "Extended error is meaningful only after the API's documented failure result and can be replaced by later Windows calls. The wrapper captures the primary OpenProcess error before cleanup, while still reporting any cleanup problem separately."),
        multiple("case1-contract-record", "foundations", "evidence", "reproducible contract", "Which records are required before the team can reproduce and explain this native-call failure?", ["The exact DLL signature, Python argtypes/restype, requested mask, and failure sentinel", "The saved PID together with process path and creation time at both observations", "The immediate result/error and the time-correlated Process Monitor event", "Only the final exception string copied from the console", "A successful test against a different process with broader rights"], [0, 1, 2], "The declaration and result contract establish how the call was made, lifetime evidence establishes which process the number referred to, and the exact result plus trace ties the internal branch to Windows. A lone exception string omits those causal links."),
        single("case1-pid-reuse", "processes-handles", "debugging", "process identity", "What is the strongest conclusion supported by the timeline before investigating the access policy?", ["PID 4180 was reused, so the later open targeted updater.exe rather than the original lab process lifetime", "The original process silently changed its executable image while retaining the same kernel object", "The PID itself retained a reference that should have prevented the first process from exiting", "ACCESS_DENIED proves updater.exe and lab_target.exe have identical security descriptors"], 0, "A PID is a reusable number rather than an owned lifetime reference. The exit and later creation records show two process objects used the same identifier, so identity must be re-established before interpreting access results."),
        ordering("case1-owned-lifecycle", "processes-handles", "ownership", "identity-bound handle lifecycle", "Order a corrected investigation that keeps process identity and ownership explicit.", [{ id: "close", label: "Close each successfully acquired snapshot and process handle exactly once in guaranteed cleanup" }, { id: "identify", label: "Start the controlled target and record PID, creation time, path, and architecture" }, { id: "open", label: "Open the target with only the rights required for the planned read-only query" }, { id: "verify", label: "Compare handle-bound identity with the recorded lifetime before interpreting later observations" }], ["identify", "open", "verify", "close"], "The target is identified before acquisition, a minimum-rights handle binds later work to a process object, identity is checked before evidence is interpreted, and every caller-owned handle is then closed exactly once.")
      ]
    },
    {
      id: "case-threaded-counter",
      title: "Faster workers, wrong result",
      modules: ["threads-scheduling", "synchronisation"],
      summary: "Separate useful I/O overlap from CPU overhead, then repair a counter whose mutex path mishandles abandoned ownership.",
      scenario: "A queue processor becomes faster up to four workers, slows at eight, and sometimes reports fewer completed items than the verified input count. One run records an abandoned mutex result after a worker is terminated.",
      artifacts: [
        { label: "Measurements", kind: "table", content: "workers  elapsed  process CPU  output\n1        4.82 s   0.71 s       10,000\n4        1.64 s   0.93 s       9,746\n8        1.91 s   1.34 s       9,881" },
        { label: "Wait trace", kind: "log", content: "worker-3 terminated while holding counter mutex\nworker-4 WaitForSingleObject -> WAIT_ABANDONED\ncode branch treated result as timeout and skipped ReleaseMutex" }
      ],
      questions: [
        single("case2-worker-shape", "threads-scheduling", "mechanism", "concurrency interpretation", "Which explanation best fits the timing curve without using the incorrect output as evidence of successful scaling?", ["I/O waits overlap through four workers, while extra scheduling and coordination costs outweigh useful overlap at eight", "Eight workers force Windows to execute every Python bytecode instruction simultaneously", "The lower counter proves the four-worker run completed more useful work in less time", "Elapsed time alone proves the workload is entirely CPU-bound"], 0, "The low process CPU relative to elapsed time supports substantial waiting, while the reversal at eight workers supports added overhead or contention. Incorrect output invalidates a performance result rather than making that run faster."),
        multiple("case2-fair-rerun", "threads-scheduling", "evidence", "controlled performance test", "Which controls belong in the rerun used to choose a worker count?", ["Use identical input and reject any run whose output count or content is wrong", "Repeat each worker count and record elapsed time, process CPU, and per-thread states", "Keep priority, partitioning, environment, and measurement boundaries fixed", "Change the queue size after each result so every worker count looks optimal", "Compare one warm run at four workers with one cold run at eight"], [0, 1, 2], "A fair comparison holds workload and environment constant, verifies correctness, and repeats timing with CPU and state evidence. Changing the queue or comparing unmatched runs introduces confounders rather than explaining the concurrency choice."),
        single("case2-abandoned-result", "synchronisation", "failure", "abandoned ownership", "What must worker 4 do after receiving WAIT_ABANDONED from the counter mutex?", ["Treat ownership as acquired, validate or repair the protected invariant, and release the mutex during guaranteed cleanup", "Treat it as a timeout because no thread owns an abandoned mutex", "Close the mutex handle immediately without examining the protected state", "Assume the counter is consistent because Windows rolled back the terminated thread's writes"], 0, "WAIT_ABANDONED grants ownership while warning that the previous owner ended without release. Windows does not restore application invariants, so the new owner validates state and must still release the ownership it acquired."),
        ordering("case2-update-order", "synchronisation", "ownership", "protected update lifecycle", "Order the corrected counter update for one worker.", [{ id: "release", label: "Release the mutex in finally only when this worker acquired ownership" }, { id: "wait", label: "Wait with a bound and branch separately on object, abandoned, timeout, and failure" }, { id: "open", label: "Open the intended named mutex with synchronization access" }, { id: "update", label: "Validate the invariant and perform the complete read-modify-write while ownership is held" }], ["open", "wait", "update", "release"], "The object is identified before waiting, the exact wait result establishes ownership and state risk, the entire invariant-changing operation runs while owned, and cleanup releases only ownership actually acquired.")
      ]
    },
    {
      id: "case-cross-architecture",
      title: "The inspector disagrees with the image",
      modules: ["memory", "linking-loading"],
      summary: "Reconcile a region walk, a PE file offset, and a 32-bit observer that cannot enumerate a 64-bit target's modules.",
      scenario: "A 32-bit Python observer can query region metadata in a 64-bit lab target, but its Tool Help module snapshot fails with ERROR_PARTIAL_COPY. A separate PE parser must locate an import record inside .rdata.",
      artifacts: [
        { label: "Region and snapshot", kind: "log", content: "caller: x86   target: x64\nVirtualQueryEx -> BaseAddress 0x00007FF700000000, RegionSize 0x3000\nCreateToolhelp32Snapshot(SNAPMODULE|SNAPMODULE32) -> INVALID_HANDLE_VALUE, error 299" },
        { label: "PE section", kind: "table", content: ".rdata VirtualAddress 0x2000\n.rdata PointerToRawData 0x0600\nImport directory RVA 0x2480\nFile size 0x9000" }
      ],
      questions: [
        single("case3-region-progress", "memory", "interface", "region-walk progress", "After the successful VirtualQueryEx result shown, which next address preserves region boundaries and forward-progress checks?", ["0x00007FF700003000, after checking pointer-sized addition for wraparound and non-progress", "0x00007FF700000001, because every byte must be queried separately", "0x0000000000003000, because RegionSize replaces BaseAddress", "The import directory RVA, because file offsets and process addresses share one coordinate system"], 0, "VirtualQueryEx describes one contiguous range, so the next query starts at BaseAddress plus RegionSize. The calculation remains pointer-sized and must reject wraparound or a result that cannot advance."),
        multiple("case3-memory-contract", "memory", "ownership", "cross-process memory safety", "Which constraints keep this read-only region investigation valid across architectures?", ["Declare addresses and sizes with pointer-width-aware types and record both architectures", "Bound the walk and validate every returned RegionSize before advancing", "Request only the process rights required by the documented metadata query", "Cast every 64-bit target address to a 32-bit integer before logging it", "Call CloseHandle on each returned BaseAddress because every address is an owned handle"], [0, 1, 2], "Architecture-aware declarations preserve address values, bounded progress prevents malformed walks, and minimum rights match the read-only goal. Region bases are target addresses, not caller-owned handles, and truncation destroys evidence."),
        single("case3-rva-offset", "linking-loading", "mechanism", "RVA translation", "What file offset should the parser test for the import directory before performing bounds checks?", ["0x0A80", "0x2A80", "0x1E80", "0x2480"], 0, "The directory lies 0x480 bytes into the section because 0x2480 minus 0x2000 equals 0x480. Adding the raw section offset 0x600 produces file offset 0xA80, which must still be checked against section and file bounds."),
        multiple("case3-compatible-fallback", "linking-loading", "failure", "architecture-compatible inspection", "Which response correctly handles the failed module snapshot without inventing module data?", ["Preserve INVALID_HANDLE_VALUE and error 299 as the snapshot outcome", "Rerun module enumeration with an architecture-compatible 64-bit observer", "Correlate the new result with target identity and image-load or module evidence", "Interpret the untouched MODULEENTRY32 buffer as a partial module record", "Assume an address resolved in the 32-bit caller names the same export in the target"], [0, 1, 2], "The failed snapshot owns no handle and produced no valid module entry. A compatible observer can repeat that phase, while identity and load evidence establish what the result means instead of transferring local addresses or stale buffers across processes.")
      ]
    },
    {
      id: "case-service-access",
      title: "The service that only an administrator can start",
      modules: ["management", "security"],
      summary: "Distinguish service transition state from access policy and restore a deliberately changed lab configuration exactly.",
      scenario: "A standard-user diagnostic can query a controlled service but cannot start it. An elevated run starts the service after two advancing checkpoints. The lab temporarily changes one value under HKCU to select a test configuration.",
      artifacts: [
        { label: "Service log", kind: "log", content: "standard token: QueryServiceStatusEx -> RUNNING\nstandard token: StartServiceW -> ACCESS_DENIED\nelevated token: START_PENDING checkpoint 4 -> 5, wait hint 2000 ms\nelevated token: RUNNING" },
        { label: "Token and Registry", kind: "table", content: "standard integrity: Medium\nelevated integrity: High\nSeDebugPrivilege: absent from standard token\nHKCU\\Software\\ILOVEOSLab\\Mode: REG_SZ 'normal' -> 'assessment'" }
      ],
      questions: [
        single("case4-pending-state", "management", "failure", "service transition", "What should the elevated controller do when the service remains START_PENDING but its checkpoint advances from 4 to 5?", ["Continue a bounded, wait-hint-aware poll because progress is occurring, while retaining an overall timeout", "Declare the service RUNNING as soon as any checkpoint changes", "Rewrite the service state directly in the Registry", "Send stop and pause controls concurrently to force a stable state"], 0, "An advancing checkpoint reports transition progress rather than stable success. A controller continues bounded observation using the current checkpoint and wait hint, while retaining a terminal timeout and handling failure states distinctly."),
        ordering("case4-registry-restore", "management", "ownership", "reversible configuration", "Order the lab's Registry mutation so the original typed value can be restored exactly.", [{ id: "restore", label: "Restore the original REG_SZ data and verify the final query, or remove only a value that was originally absent" }, { id: "record", label: "Query and record whether the exact value exists together with its type and data" }, { id: "change", label: "Open only the disposable HKCU path with the minimum set-value access and write the test value" }, { id: "observe", label: "Run the controlled service query and capture the result before cleanup" }], ["record", "change", "observe", "restore"], "Reversibility begins with exact prior state, limits mutation to the disposable value, preserves the observation made under that condition, and distinguishes restoring typed data from deleting something created only by the lab."),
        multiple("case4-access-evidence", "security", "evidence", "authorization diagnosis", "Which evidence is needed to explain why query succeeds while start is denied for the standard token?", ["The exact SCM/service access masks requested by the two operations", "The standard token's groups, integrity, elevation, and relevant privileges", "The controlled service object's security descriptor or an AccessChk result", "The fact that both attempts used an executable with the same filename", "Total machine CPU use during the request"], [0, 1, 2], "Authorization depends on the acting token, exact requested rights, and target policy. A shared executable name and system load do not connect the subject, requested operation, and service security descriptor that produced the different decisions."),
        single("case4-privilege-conclusion", "security", "debugging", "privilege evidence", "The standard token does not contain SeDebugPrivilege. What is the contract-correct response if the diagnostic attempts to enable it?", ["Expect ERROR_NOT_ALL_ASSIGNED, report that the privilege is absent, and continue only with operations allowed by the existing token", "Insert the missing privilege into the token because AdjustTokenPrivileges creates new privileges", "Treat the API's normal return value as proof that the absent privilege became enabled", "Replace the service DACL because token privileges and access-control entries are interchangeable"], 0, "AdjustTokenPrivileges changes attributes of privileges already present; it does not add a missing privilege. The special last-error result and a token re-query establish the outcome without silently escalating or confusing privilege state with object policy.")
      ]
    },
    {
      id: "case-pipe-observer",
      title: "The observer that never reaches EOF",
      modules: ["ipc", "hooking-injection"],
      summary: "Repair a hanging evidence channel and decide what an unexpected image-load event does, and does not, prove.",
      scenario: "A parent captures JSON messages from a purpose-built observer through a pipe. The child exits, but the parent blocks waiting for EOF. Before exit, Process Monitor recorded an unexpected DLL load in the controlled target.",
      artifacts: [
        { label: "Handle trace", kind: "log", content: "parent created read=0x74, write=0x78\nchild inherited write=0x78\nparent retained write=0x78 after CreateProcess\nchild closed write and exited\nparent ReadFile(read=0x74) remains blocked" },
        { label: "Evidence fragment", kind: "log", content: "ReadFile -> 96 bytes + ERROR_MORE_DATA\nProcmon: 15:42:10.411 target.exe Load Image C:\\Lab\\plugins\\telemetry.dll\nmodule snapshot at 15:42:11 includes telemetry.dll; signer: unavailable" }
      ],
      questions: [
        single("case5-pipe-eof", "ipc", "ownership", "pipe endpoint lifetime", "Which ownership error directly explains why the parent's read does not reach EOF after the child exits?", ["The parent retained an unused write-end copy, so at least one writer still exists", "The child closed its intended write handle before terminating", "The parent owns a read handle, and readers themselves suppress EOF", "ERROR_MORE_DATA permanently converts the pipe into a mutex"], 0, "EOF requires every write endpoint to close after buffered data is consumed. The parent's forgotten write copy keeps the stream open even though the intended child writer closed its handle and exited."),
        multiple("case5-message-assembly", "ipc", "failure", "partial message handling", "How should the parent handle the 96-byte fragment accompanied by ERROR_MORE_DATA?", ["Keep the fragment as part of the current message", "Continue bounded reads until that message completes", "Decode only after complete byte assembly using the agreed encoding", "Discard all returned bytes because any error code means the buffer is empty", "Decode each fragment independently even if a character spans buffers"], [0, 1, 2], "Message-mode reads can return valid bytes with ERROR_MORE_DATA when the supplied buffer is smaller than one message. Bounded accumulation preserves framing, and decoding after assembly protects multibyte character boundaries."),
        single("case5-load-claim", "hooking-injection", "evidence", "load-event limits", "What does the Process Monitor event plus the later module snapshot establish?", ["The target mapped telemetry.dll around the recorded time and it remained present at the snapshot, but the loading mechanism and intent remain unproven", "A remote thread definitely injected telemetry.dll and executed a malicious export", "The unavailable signer field proves the image has no valid signature", "The module was present from process creation because it appears in one later snapshot"], 0, "The trace supports a time-associated image load, and the snapshot supports later presence. Neither observation alone identifies the initiating mechanism, signer verdict, persistence before the event, or intent; those require additional controlled evidence."),
        ordering("case5-defensive-baseline", "hooking-injection", "evidence", "controlled module investigation", "Order the next defensive investigation without modifying the target or unrelated processes.", [{ id: "cleanup", label: "Close observer, process, snapshot, and pipe handles and remove only artifacts created by the lab" }, { id: "baseline", label: "Restart the owned target and record identity, architecture, modules, paths, signers, and capture start time" }, { id: "trigger", label: "Trigger only the controlled feature and collect a second module snapshot" }, { id: "correlate", label: "Correlate additions with image-load events, full paths, timestamps, and documented application behavior" }], ["baseline", "trigger", "correlate", "cleanup"], "A complete identity-aware baseline precedes one controlled change. Snapshot differences are then correlated with trace and file evidence, and the observer cleans up its own resources without writing into the target or asserting intent beyond the evidence.")
      ]
    }
  ];

  const finalQuestions = finalCases.flatMap((caseFile) => caseFile.questions.map((question) => ({ ...question, caseId: caseFile.id })));
  ...
  window.ILOVEOS_ASSESSMENTS = {
    ...
    finalAssessment: {
      ...
      summary: "Five integrated cases test diagnosis, evidence, interface contracts, failure handling, and ownership across all ten modules.",
      cases: finalCases,
