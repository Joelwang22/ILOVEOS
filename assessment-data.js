(() => {
  ...
  function single(id, module, dimension, skill, prompt, options, answer, explanation) {
    return { id, module, kind: "single", dimension, skill, prompt, options, answer, explanation };
  }

  function multiple(id, module, dimension, skill, prompt, options, answers, explanation) {
    return { id, module, kind: "multiple", dimension, skill, prompt, options, answers, explanation };
  }
    ...
    },
    {
      ...
      activities: [
        ...
        ordering(
          ...
          "Order a read-only service query that uses native SCM handles and guarantees correct cleanup.",
          [
            { id: "close-scm", label: "Close the SCM handle after every service handle obtained from it has been closed" },
            ...
            { id: "close-service", label: "Close the service handle in guaranteed cleanup" }
          ],
          ["open-scm", "open-service", "query", "close-service", "close-scm"],
          "The service handle depends on an open SCM connection and is closed first in the reverse ownership order. Status and error evidence must be captured before cleanup calls can replace thread-local error state."
        ),
        single(
  ...
  const finalQuestions = [
    ...
    single("final-processes-identity", "processes-handles", "failure", "PID reuse diagnosis", "A stored PID now refers to a different process. Which missing evidence would have prevented the mistaken identity?", ["Creation time and a retained handle or other lifetime-bound identity evidence", "The executable's desktop shortcut color", "The system page size", "The number of Registry hives"], 0, "PIDs are reusable identifiers. Creation time, parent/command line, and especially a retained handle tied to the original object distinguish one process lifetime from another later process using the same number."),
    ...
    multiple("final-security-privilege", "security", "failure", "privilege failure", "Which observations prove a requested privilege adjustment did not fully succeed?", ["ERROR_NOT_ALL_ASSIGNED after the documented success-shaped call", "The privilege is absent from the token", "A re-query shows the privilege not enabled", "The process name contains lowercase letters", "The target file is on an SSD"], [0, 1, 2], "The API's special last-error branch, privilege presence, and resulting attributes directly address the adjustment. Process naming and storage hardware are unrelated to whether the token held and enabled that privilege."),
    ...
    ordering("final-sync-order", "synchronisation", "ownership", "synchronized update", "Order the essential steps for a bounded mutex-protected update.", [{ id: "release", label: "Release if acquired" }, { id: "update", label: "Validate and update protected state" }, { id: "wait", label: "Wait and branch on the exact result" }, { id: "open", label: "Open the intended mutex" }], ["open", "wait", "update", "release"], "Object identity comes first, then the wait result establishes whether ownership exists. Only an owning branch touches protected state, and guaranteed cleanup releases exactly that acquired ownership."),
  ...
  const practical = {
    ...
    scenario: "You maintain a diagnostic Python utility for a purpose-built lab process that you own. The utility records process identity, enumerates virtual-memory regions and loaded modules, and correlates the result with Process Explorer and a short Process Monitor trace. On a clean 64-bit lab VM, the first run succeeds. A second run launched from a 32-bit Python environment opens the process but cannot interpret one module structure correctly, and a deliberately over-broad process-open attempt reports access denied. Design a controlled investigation that explains both outcomes without modifying target memory, starting remote execution, disabling protections, or testing unrelated processes.",
    prompts: [
      { id: "contract", label: "Choose the contract", prompt: "Select the minimum process rights and the pywin32 or native query APIs needed for identity, region, and module evidence. State each result sentinel, error rule, and output structure." },
      ...
      { id: "outcomes", label: "Predict branches", prompt: "Predict the success, access-denied, partial or unavailable information, process-exit, and address-walk termination branches that the utility must report distinctly." },
    ...
    modelReasoning: [
      ...
      { id: "model-rights", title: "Request minimum rights", body: "Open only the owned target and request the query/read capabilities required by the chosen APIs. Report the over-broad denial as expected policy evidence rather than escalating automatically." },
      { id: "model-abi", title: "Treat ABI as evidence", body: "Record both architectures and use pointer-sized fields and the structure variant required by the contract. Never assume a local pointer or module base identifies the same remote address." },
      { id: "model-results", title: "Branch on documented results", body: "Distinguish open denial, target exit, query termination, partial module visibility, incompatible structure interpretation, and successful completion. Capture extended error immediately only on the documented branch." },
      ...
      { id: "model-ownership", title: "Close what you own", body: "Retain and close every real process or snapshot handle exactly once, allow Python-managed buffers to expire after calls complete, and do not close borrowed identifiers or pseudo-handles." },
