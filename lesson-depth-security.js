window.ILOVEOS_LESSON_DEPTH = {
  ...
  "security-model": {
    ...
    practice: {
      ...
      expectedOutcome: "The owned Notepad target should expose one stable PID and creation time. Both read-only OpenProcess probes may succeed, but their output must show the difference between PROCESS_ALL_ACCESS (0x001F0FFF) and the image-query minimum PROCESS_QUERY_LIMITED_INFORMATION (0x1000).",
      ...
      steps: [
        ...
        { action: "In the same ordinary PowerShell, use this read-only ctypes probe to request PROCESS_ALL_ACCESS (0x001F0FFF); it closes any returned handle without reading or changing process memory.", commands: [{ label: "PowerShell", code: "$target = Get-Process -Id $targetPid -ErrorAction Stop\n\"Target: $($target.ProcessName), PID=$targetPid, creation=$($target.StartTime.ToString('o'))\"\n@'\nimport ctypes, sys\nfrom ctypes import wintypes\npid = int(sys.argv[1])\nmask = 0x001F0FFF\nkernel32 = ctypes.WinDLL('kernel32', use_last_error=True)\nkernel32.OpenProcess.argtypes = [wintypes.DWORD, wintypes.BOOL, wintypes.DWORD]\nkernel32.OpenProcess.restype = wintypes.HANDLE\nkernel32.CloseHandle.argtypes = [wintypes.HANDLE]\nkernel32.CloseHandle.restype = wintypes.BOOL\nhandle = kernel32.OpenProcess(mask, False, pid)\nerror = ctypes.get_last_error()\nprint(f'OpenProcess PID={pid} requested=0x{mask:08X} handle={int(handle or 0):#x} error={error}: {ctypes.FormatError(error) if error else \"success\"}')\nif handle:\n    kernel32.CloseHandle(handle)\n'@ | py - $targetPid" }], why: "The exact API, object, requested mask, and Win32 result form a reproducible access contract.", observe: "The line shows handle, numeric error, and message. Error 5 is access denied; a nonzero handle is success and is closed. An exited PID is a target-state failure, not a DACL conclusion." },
        ...
        { action: "In the same PowerShell, repeat the controlled open with only PROCESS_QUERY_LIMITED_INFORMATION (0x1000).", commands: [{ label: "PowerShell", code: "$target = Get-Process -Id $targetPid -ErrorAction Stop\n\"Target: $($target.ProcessName), PID=$targetPid, creation=$($target.StartTime.ToString('o'))\"\n@'\nimport ctypes, sys\nfrom ctypes import wintypes\npid = int(sys.argv[1])\nmask = 0x00001000\nkernel32 = ctypes.WinDLL('kernel32', use_last_error=True)\nkernel32.OpenProcess.argtypes = [wintypes.DWORD, wintypes.BOOL, wintypes.DWORD]\nkernel32.OpenProcess.restype = wintypes.HANDLE\nkernel32.CloseHandle.argtypes = [wintypes.HANDLE]\nkernel32.CloseHandle.restype = wintypes.BOOL\nhandle = kernel32.OpenProcess(mask, False, pid)\nerror = ctypes.get_last_error()\nprint(f'OpenProcess PID={pid} requested=0x{mask:08X} handle={int(handle or 0):#x} error={error}: {ctypes.FormatError(error) if error else \"success\"}')\nif handle:\n    kernel32.CloseHandle(handle)\n'@ | py - $targetPid" }], why: "A smaller successful request demonstrates the minimum authority for the image-path query.", observe: "The requested field is 0x00001000 and the owned live target normally returns a nonzero handle with error 0. If it fails, the exact Win32 error remains visible; no privilege or DACL change follows." }
      ],
  ...
  "privileges-impersonation": {
    ...
    practice: {
      ...
      time: "55 min",
      intro: "Run privilege_state_lab.py read-only first. Keep token_launch_lab.py in a VM and use only a process owned by your current account.",
      ...
      expectedOutcome: "privilege_state_lab.py reports whether a named privilege is present and enabled, and restores any temporary enable. token_launch_lab.py defaults to dry-run, compares source and current SIDs, creates one primary duplicate, then closes it. With --launch and sufficient policy, it creates one same-user child and closes the returned process and thread handles.",
      ...
      steps: [
        ...
        { action: "Only in the disposable VM, create an owned cmd.exe source process and pass its printed PID directly to token_launch_lab.py without --launch.", commands: [{ label: "PowerShell (VM only)", code: "$source = Start-Process -FilePath $env:ComSpec -ArgumentList '/k title ILOVEOS Token Source' -PassThru\n\"Owned source PID: $($source.Id)\"\n\"Owned source creation time: $($source.StartTime.ToString('o'))\"\npy .\\token_launch_lab.py $source.Id" }], why: "Dry-run validates PROCESS_QUERY_LIMITED_INFORMATION, token query/duplicate rights, same-SID scope, one primary duplicate, and cleanup before execution.", observe: "The output shows matching Source SID and Current SID, 'Created one owned primary-token duplicate', and 'Dry run complete'; no child PID appears. A redirected/exited source, access denial, or different SID stops the branch." },
        { action: "Only in the disposable VM, enter the still-live Owned source PID from the dry run and request token_launch_lab.py's benign notepad.exe launch.", commands: [{ label: "Authorised PowerShell (VM only)", code: "$sourcePid = [int](Read-Host 'Enter the still-live Owned source PID from the dry run')\nGet-Process -Id $sourcePid -ErrorAction Stop | Select-Object Id,ProcessName,StartTime\npy .\\token_launch_lab.py $sourcePid notepad.exe --launch" }], why: "The final observation tests primary-token process creation without crossing account boundaries.", observe: "The console shows child PID/TID or the exact CreateProcessAsUser failure. For a child PID, select it in Process Explorer's top pane and open Properties > Security to view user SID and integrity. Missing or disabled privilege, access denial, UAC cancellation, exited source, and immediately exited/redirected child remain separate branches." }
      ],
      hints: [{ title: "CreateProcessAsUser returns privilege not held", body: "privilege_state_lab.py distinguishes absent from disabled state. Do not switch to a higher-value target or weaken security policy to force success." }],
