  ...
  "privileges-impersonation": {
    ...
    practice: {
      ...
      steps: [
        ...
        { action: "Create one owned same-user cmd.exe source, pass its PID as token_launch_lab.py's only argument, and leave the source window open until the dry run finishes.", commands: [{ label: "PowerShell", code: "$source = Start-Process -FilePath $env:ComSpec -ArgumentList '/k title ILOVEOS Token Source' -PassThru\n\"Owned source PID: $($source.Id)\"\n\"Owned source creation time: $($source.StartTime.ToString('o'))\"\nGet-Process -Id $source.Id -ErrorAction Stop | Select-Object Id,ProcessName,StartTime,Path\npy .\\token_launch_lab.py $source.Id" }], why: "The dry run validates PROCESS_QUERY_LIMITED_INFORMATION, token query/duplicate rights, same-SID scope, one primary duplicate, and cleanup without exposing a process-creation mode.", observe: "The output shows matching Source SID and Current SID, 'Created one owned primary-token duplicate', 'Dry run evidence: child process created=no', four 'Closed owned handle' lines including 'Closed owned handle: primary token', and 'Cleanup evidence: owned handles closed=4'. An exited source, access denial, or different SID stops the branch without switching targets." }
      ],
      ...
      cleanup: ["After token_launch_lab.py prints 'Cleanup evidence: owned handles closed=4', close the owned ILOVEOS Token Source cmd.exe window.", "privilege_state_lab.py restores any temporary privilege before it exits, and token_launch_lab.py has already printed the closure of every owned process/token handle."],
    },
    checks: [
