(() => {
  ...
  const familyReview = {
    ...
    OpenFileMappingW: review("encoding", "OpenFileMapping", "https://learn.microsoft.com/en-us/windows/win32/api/memoryapi/nf-memoryapi-openfilemappingw", "The A/W forms differ only in the named mapping lookup string."),
  ...
  const nativeDefinitions = {
    ...
    PAGE_FILE_BACKING: "ctypes.c_void_p(-1).value",
    ...
    AUTO_RESET_NONSIGNALED: "0",
    CREATE_EVENT_MANUAL_RESET: "0x00000001",
    CREATE_EVENT_INITIAL_SET: "0x00000002",
    EVENT_MODIFY_STATE: "0x00000002",
    NO_INITIAL_OWNERSHIP: "0",
  ...
  const choiceSelectionGroups = {
    "named-pipe-open-mode": [
      { id: "access-direction", names: ["PIPE_ACCESS_INBOUND", "PIPE_ACCESS_OUTBOUND", "PIPE_ACCESS_DUPLEX"] },
    ],
    "named-pipe-mode": [
      { id: "pipe-type", names: ["PIPE_TYPE_BYTE", "PIPE_TYPE_MESSAGE"] },
      { id: "read-mode", names: ["PIPE_READMODE_BYTE", "PIPE_READMODE_MESSAGE"] },
      { id: "wait-mode", names: ["PIPE_WAIT", "PIPE_NOWAIT"] },
    ],
    "message-box-options": [
      { id: "button-set", names: ["MB_OK", "MB_OKCANCEL", "MB_YESNO"] },
      { id: "icon", names: ["MB_ICONINFORMATION", "MB_ICONWARNING"] },
    ],
    "final-path-format": [
      { id: "file-name", names: ["FILE_NAME_NORMALIZED", "FILE_NAME_OPENED"] },
      { id: "volume-name", names: ["VOLUME_NAME_DOS", "VOLUME_NAME_GUID", "VOLUME_NAME_NT", "VOLUME_NAME_NONE"] },
    ],
    "event-log-read-flags": [
      { id: "read-position", names: ["EVENTLOG_SEQUENTIAL_READ", "EVENTLOG_SEEK_READ"] },
      { id: "read-direction", names: ["EVENTLOG_FORWARDS_READ", "EVENTLOG_BACKWARDS_READ"] },
    ],
    "evt-subscribe-flags": [
      { id: "subscription-start", names: ["EvtSubscribeToFutureEvents", "EvtSubscribeStartAtOldestRecord", "EvtSubscribeStartAfterBookmark"] },
    ],
    "evt-query-flags": [
      { id: "query-source", names: ["EvtQueryChannelPath", "EvtQueryFilePath"] },
      { id: "query-direction", names: ["EvtQueryForwardDirection", "EvtQueryReverseDirection"] },
    ],
    "evt-export-flags": [
      { id: "export-source", names: ["EvtExportLogChannelPath", "EvtExportLogFilePath"] },
    ],
  };
  const choiceStandaloneValues = {
    "memory-allocation-type": ["MEM_RESET"],
    "file-flags": ["FILE_ATTRIBUTE_NORMAL"],
    "file-attributes": ["FILE_ATTRIBUTE_NORMAL"],
    "cert-query-content-type": ["CERT_QUERY_CONTENT_FLAG_ALL"],
    "cert-query-format-type": ["CERT_QUERY_FORMAT_FLAG_ALL"],
  };
  ...
  function resolveParameterChoices(bindingKey, surface) {
    ...
    const resolvedValues = names.filter((name) => choiceSet.values?.[name]).map((name) => {
      const group = (choiceSelectionGroups[binding.choiceSet] || []).find((item) => item.names.includes(name));
      const code = choiceSet.values[name][surface === "pywin32" ? "pywin32" : "native"];
      return {
        name,
        code,
        definition: surface === "native" ? nativeDefinitions[name] || "" : "",
        control: choiceSet.kind === "enum" || choiceSet.kind === "sentinel" || group ? "radio" : "checkbox",
        group: group?.id || (choiceSet.kind === "enum" || choiceSet.kind === "sentinel" ? "single-value" : ""),
        zero: surface === "native" ? nativeDefinitions[name] === "0" : code === "0",
        standalone: (choiceStandaloneValues[binding.choiceSet] || []).includes(name),
        useWhen: choiceSet.values[name].useWhen,
      };
    });
    const exampleParts = typeof binding.example?.code === "string" ? binding.example.code.split("|").map((part) => part.trim()) : [];
    const example = binding.example && typeof binding.example.code === "string" && binding.example.code.trim()
      ? {
        code: binding.example.code,
        names: resolvedValues.filter((value) => exampleParts.includes(value.name) || exampleParts.includes(value.code)).map((value) => value.name),
        useWhen: binding.example.useWhen || "",
      }
      : null;
    return {
      ...
      kind: choiceSet.kind,
      surface,
      parameter: bindingKey.split(".").at(-1) || "value",
  ...
  window.ILOVEOS_WINDOWS_API_FAMILY_DATA = {
    ...
    choiceSets,
    nativeDefinitions,
    choiceSelectionGroups,
    choiceStandaloneValues,
    nativeBindings,
    pywin32Bindings,
    buildFamilies,
    resolveSelection,
