window.ILOVEOS_LESSON_DEPTH = {
  ...
  "registry-structure": {
    ...
    practice: {
      ...
      steps: [
        ...
        { action: "Return to ordinary PowerShell and paste this bounded cleanup block, then refresh the exact Registry Editor path.", commands: [{ label: "PowerShell cleanup", code: "$labRoot = 'HKCU:\\Software\\ILOVEOSLab'\n$labKey = Join-Path $labRoot 'Structure'\nif (Test-Path -LiteralPath $labKey) {\n  Remove-ItemProperty -LiteralPath $labKey -Name DemoMode -ErrorAction SilentlyContinue\n  if (-not (Get-ChildItem -LiteralPath $labKey -Force)) { Remove-Item -LiteralPath $labKey }\n}\nif ((Test-Path -LiteralPath $labRoot) -and -not (Get-ChildItem -LiteralPath $labRoot -Force)) { Remove-Item -LiteralPath $labRoot }\n\"Structure key present after cleanup: $(Test-Path -LiteralPath $labKey)\"" }], why: "Cleanup names each owned target and preserves any unexpected sibling item.", observe: "PowerShell prints Structure key present after cleanup: False. Registry Editor no longer shows Structure; if an unexpected sibling prevented parent removal, it remains preserved." }
      ],
  ...
  "control-services-python": {
    ...
    practice: {
      ...
      time: "50 min",
      intro: "Start service_controller_lab.py in query mode. Change state only for a documented non-critical target in a disposable VM.",
      ...
      safety: "Do not stop security, storage, networking, logon, update, endpoint-protection, hypervisor, or other critical services. Use only a pre-authorised lab service in a disposable VM; the supplied block keeps and restores its stable starting state.",
      steps: [
        { action: "Open service_controller_lab.py and locate QueryServiceStatusEx, action-specific access, the --confirm gate, 30-second bounded waiting, progress fields, and reverse-order CloseServiceHandle calls.", why: "The visible control and cleanup paths establish the artifact's safety contract before a service is named.", observe: "The file contains state, checkpoint, wait-hint, and PID progress plus separate query, start, and stop access masks. No repository-only comparison file is required." },
        { action: "Download service_controller_lab.py, open ordinary PowerShell in its folder, enter the internal name of the pre-authorised non-critical VM lab service, and run query mode.", commands: [{ label: "PowerShell", code: "$serviceName = Read-Host 'Enter the internal name of the pre-authorised non-critical VM lab service'\npy .\\service_controller_lab.py query $serviceName" }], why: "Query mode validates the exact name, ctypes signatures, handle ownership, and SERVICE_QUERY_STATUS access without changing state.", observe: "The Before row shows state and PID. OpenSCManagerW, OpenServiceW, or QueryServiceStatusEx errors name the failed operation; error 5 is insufficient rights, while error 1060 is an absent or incorrect service name." },
        { action: "From ordinary PowerShell, enter start or stop and run service_controller_lab.py without --confirm.", commands: [{ label: "PowerShell", code: "$serviceName = Read-Host 'Enter the internal VM lab service name'\n$action = (Read-Host 'Enter exactly start or stop').ToLowerInvariant()\nif ($action -notin @('start','stop')) { throw 'Action must be start or stop.' }\npy .\\service_controller_lab.py $action $serviceName\nGet-Service -Name $serviceName | Select-Object Name,Status" }], why: "The explicit action boundary protects against accidental changes while arguments are checked.", observe: "The console shows the Before row and 'No state change made'; Get-Service shows the unchanged Status. An access-denied open can occur before the confirmation gate because the action-specific handle right is still requested." },
        { action: "Only in the disposable VM, open authorised elevated PowerShell in the folder containing service_controller_lab.py and paste this block. It captures Running or Stopped in memory, performs the confirmed transition, and restores that starting state in finally.", commands: [{ label: "Elevated PowerShell change and restoration (VM only)", code: "$serviceName = Read-Host 'Enter the internal VM lab service name'\n$action = (Read-Host 'Enter exactly start or stop').ToLowerInvariant()\nif ($action -notin @('start','stop')) { throw 'Action must be start or stop.' }\n$startingState = (Get-Service -Name $serviceName -ErrorAction Stop).Status.ToString()\nif ($startingState -notin @('Running','Stopped')) { throw \"Starting state must be stable, not $startingState\" }\ntry {\n  py .\\service_controller_lab.py $action $serviceName --confirm\n  Get-Service -Name $serviceName | Select-Object Name,Status\n} finally {\n  $restoreAction = if ($startingState -eq 'Running') { 'start' } else { 'stop' }\n  $currentState = (Get-Service -Name $serviceName -ErrorAction Stop).Status.ToString()\n  if ($currentState -ne $startingState) { py .\\service_controller_lab.py $restoreAction $serviceName --confirm }\n  Get-Service -Name $serviceName | Select-Object Name,Status\n  \"Restored starting state: $startingState\"\n}" }], why: "The command keeps the original state and cleanup target inside one reversible operation.", observe: "Progress rows show state, checkpoint, wait hint, and PID; the last Get-Service row and Restored starting state line agree. UAC cancellation, error 5, rejected control, timeout, and restoration failure remain distinct visible branches and must not trigger broader rights or process termination." }
      ],
      ...
      cleanup: ["The elevated block restores the exact stable starting state in finally.", "service_controller_lab.py closes both SCM handles on success and failure paths."],
    },
    checks: [
  ...
  "wow64-redirection": {
    ...
    practice: {
      ...
      steps: [
        ...
        { action: "If both interpreter architectures are installed, enter each python.exe path and run wow64_views_lab.py inside a wrapper that prints its PID and pauses after its report; inspect 32-bit first, close it, then repeat for 64-bit.", commands: [{ label: "Architecture comparison PowerShell", code: "$python32 = Read-Host 'Enter the full path to 32-bit python.exe, or leave blank if unavailable'\n$python64 = Read-Host 'Enter the full path to 64-bit python.exe, or leave blank if unavailable'\nforeach ($pythonExe in @($python32,$python64)) {\n  if ([string]::IsNullOrWhiteSpace($pythonExe)) { 'Interpreter architecture unavailable; skipped.'; continue }\n  if (-not (Test-Path -LiteralPath $pythonExe -PathType Leaf)) { throw \"python.exe not found: $pythonExe\" }\n  & $pythonExe -c \"import os, runpy; print(f'Python PID: {os.getpid()}'); runpy.run_path(r'.\\wow64_views_lab.py', run_name='__main__'); input('Paused for Process Explorer; press Enter to exit...')\"\n}" }], why: "A controlled pair exposes pointer width, explicit view results, and architecture-matched module selection.", observe: "For each printed PID, select that process in Process Explorer's top pane, choose View > Show Lower Pane and View > Lower Pane View > DLLs, right-click a lower-pane column header, choose Select Columns, open DLL, enable Path, and locate the loaded Python DLL row. wow64_views_lab.py shows pointer width; the exact DLL row shows its module path. If only one architecture exists or lower-pane access is denied, that comparison branch is unavailable." }
      ],
