window.ILOVEOS_LESSON_DEPTH = {
  ...
  "control-services-python": {
    ...
    practice: {
      ...
      steps: [
        { action: "Open service_controller_lab.py and locate the query action, QueryServiceStatusEx structure, SERVICE_QUERY_STATUS access, printed Before state and PID, and reverse-order CloseServiceHandle calls.", why: "The artifact shows that query mode has its own narrow access and ownership path.", observe: "The query branch opens EventLog with status-query access, prints the returned state and PID once, and reaches the same service-then-SCM cleanup path. Mutating branches remain visible in service_controller_lab.py but are outside this required showcase." },
  ...
  "wow64-redirection": {
    ...
    workedExamples: [{ type: "decision", title: "Choose the correct architecture question", prompt: "A 32-bit Python management tool runs on x64 Windows.", steps: [
      ...
      { title: "Check the native host architecture", action: "Use IsWow64Process2 or a native-system query.", why: "The compatibility process view may differ from the host.", result: "x64 host" },
      { title: "Choose the Registry view explicitly", action: "Add KEY_WOW64_32KEY or KEY_WOW64_64KEY to the requested mask.", why: "Explicit selection prevents interpreter defaults from deciding silently.", result: "Known logical view" },
      { title: "Verify which file Windows opened", action: "Capture the operation in Process Monitor.", why: "A hard-coded folder-name rule cannot cover every exclusion and version.", result: "Observed resolved path" }
