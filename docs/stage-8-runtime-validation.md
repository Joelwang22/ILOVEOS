...
The separate [theoretical runtime audit](stage-8-theoretical-validation.md) checks all 34 pending implementations against primary Win32, CPython, pywin32, and Sysinternals references and records successful command-surface, ABI, constant, and read-only probes. It found no blocking contract contradiction and concludes that the documented normal paths are expected to pass. The rows remain pending because theoretical confidence is not execution evidence.
