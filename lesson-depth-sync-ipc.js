window.ILOVEOS_LESSON_DEPTH = {
  ...
  "atomicity-races": {
    ...
    checks: [
      ["Why is the increment shown here unsafe when two workers run it concurrently?", ["The number is written in decimal", "The read and write steps can interleave", "The threads have separate PIDs", "Waiting closes the handles"], 1, "Both workers can read the same old value and then overwrite each other's update."],
  ...
  "events-waits": {
    ...
    checks: [
      ...
      ["Which access right does code need if it only waits for an event?", ["SYNCHRONIZE", "PROCESS_ALL_ACCESS", "EVENT_MODIFY_STATE only", "GENERIC_WRITE"], 0, "SYNCHRONIZE allows the caller to wait without granting permission to change the event's state."]
    ]
  },
  ...
  "deadlocks-starvation": {
    ...
    checks: [
      ...
      ["Which change prevents threads from forming a circular wait?", ["Raising their priorities", "Requiring one consistent lock-acquisition order", "Adding more threads", "Using longer sleeps"], 1, "A consistent acquisition order prevents a dependency cycle among the locks that follow it."]
    ]
  },
  ...
  "named-pipes": {
    ...
    practice: {
      ...
      steps: [
        { action: "Download named_pipe_lab.py and open two PowerShell windows in its folder. Start the server first and leave it paused at 'Inspect Process Monitor and Handle'. In Process Monitor, include paths containing ILOVEOS_MessageLab, clear the display, and start capturing. Press Enter in the server so it waits in ConnectNamedPipe, then start the client.", commands: [{ label: "Server PowerShell", code: "py .\\named_pipe_lab.py server" }, { label: "Client PowerShell", code: "py .\\named_pipe_lab.py client Heisenberg --timeout-ms 5000" }], why: "Starting the two sides in this order makes server creation, availability, connection, framing, and closure visible.", observe: "The console and trace show \\\\.\\pipe\\ILOVEOS_MessageLab, both PIDs, and the CreateFile, ReadFile, WriteFile, close, and disconnect events. The server receives 'Heisenberg', and the client receives 'Hello, Heisenberg'. If Process Monitor denies access to the details, that evidence is unavailable." },
  ...
  "choose-ipc": {
    ...
    learning: [
      { title: "Choose the mechanism from constraints", paragraphs: ["Anonymous pipes are strong for related process streams and standard-handle redirection. Named pipes suit discoverable local client-server channels and can support remote use. Shared mappings suit high-volume local data when you can design synchronization. Sockets suit network and cross-platform channels. Files suit durable, decoupled transfer.", "Higher-level RPC and messaging libraries add schemas, marshaling, authentication, discovery, cancellation, and version evolution. Their overhead can remove risky protocol work. Use the lowest level only when its control or learning value is required."], inlineCheck: ["When is a file a better IPC choice than an anonymous pipe?", ["When the data must persist independently of both processes", "When redirecting a child's stdout", "When sending one event signal", "When assigning thread ownership"], 0, "A file can outlive both participants and lets them exchange data without running at the same time."] },
