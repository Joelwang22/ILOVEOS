window.ILOVEOS_LESSON_DEPTH = {
  ...
  "events-waits": {
    ...
    practice: {
      ...
      steps: [
        { action: "Download event_pair_lab.py and open three PowerShell windows in its folder. Start Creator PowerShell first and leave it blocked at 'Start waiter processes'; run the Waiter PowerShell block separately in each other window, where both block in WaitForSingleObject.", commands: [{ label: "Creator PowerShell", code: "@'\nimport os\nimport runpy\nimport sys\nprint(f'Creator PID: {os.getpid()}')\nsys.argv = ['event_pair_lab.py', 'creator']\nrunpy.run_path(r'.\\event_pair_lab.py', run_name='__main__')\n'@ | py -" }, { label: "Waiter PowerShell", code: "@'\nimport os\nimport runpy\nimport sys\nprint(f'Waiter PID: {os.getpid()}')\nsys.argv = ['event_pair_lab.py', 'waiter', '--timeout-ms', '10000']\nrunpy.run_path(r'.\\event_pair_lab.py', run_name='__main__')\n'@ | py -" }], why: "The startup order establishes one manual-reset Local\\ILOVEOS_InvitationLab object with creator ownership and two SYNCHRONIZE waiter handles.", observe: "Before signalling, select each printed PID in Process Explorer's top pane, choose View > Show Lower Pane and View > Lower Pane View > Handles, and search ILOVEOS_InvitationLab; access denial is an unavailable branch. Press Enter once in Creator PowerShell and both waiters print 'invitation received'; a 10000 ms expiry prints 'invitation timed out'." },
  ...
  "anonymous-pipes": {
    ...
    practice: {
      ...
      steps: [
        ...
        { action: "Run pipeline_lab.py with a 200000-byte producer and a pass-through findstr consumer.", commands: [{ label: "PowerShell", code: "$producer = 'powershell.exe -NoProfile -Command \"[Console]::Out.Write((''X'' * 200000))\"'\n$consumer = 'findstr.exe X'\npy .\\pipeline_lab.py $producer $consumer" }], why: "Output larger than READ_BUFFER_SIZE tests that the parent drains final_read before waiting for child exit.", observe: "Require 'parent captured 200000 bytes before waiting' and eventual exit rows rather than a bounded-buffer deadlock. The final printed payload is intentionally large; interrupt only the lab children if a deliberately altered ownership variant hangs." },
